...
plot_aggregate_biomass <- function(shadedRegion = NULL,
  ...
  agg<-ecodata24::aggregate_biomass |>
  ...
  if(report == "MidAtlantic"){
    #Get NEAMAP
    neamap <- ecodata24::mab_inshore_survey |>
  ...
  p1<-agg_bio |>
    ...
    ecodata24::geom_gls(ggplot2::aes(x = Time, y = Mean,
                                   color = Var),
                      alpha = setup$trend.alpha, size = setup$trend.size) +
    # ecodata24::geom_lm(aes(x = Time, y = Mean,
    #              color = Var),
  ...
  p3<-agg_bio |>
    ...
    ecodata24::geom_gls_gauss(ggplot2::aes(x = Time, y = Mean,
                                   color = Var),
                      alpha = setup$trend.alpha, size = setup$trend.size) +
    # ecodata24::geom_lm(aes(x = Time, y = Mean,
    #              color = Var),
...
#                     ymin = -Inf, ymax = Inf) +
#   #Test for trend and add lines
#   ecodata24::geom_gls(aes(x = Time, y = Mean,
#                         color = Var),
#                     alpha = trend.alpha, size = trend.size) +
#   # ecodata24::geom_lm(aes(x = Time, y = Mean,
#   #              color = Var),
