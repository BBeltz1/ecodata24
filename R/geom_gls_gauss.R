...
geom_gls_gauss <- function(mapping = NULL, data = NULL, stat = "GLSgauss",
                     position = "identity", na.rm = FALSE, show.legend = NA,
                     inherit.aes = TRUE, warn = TRUE, ...) {
  ggplot2::layer(
    geom = ecodata24:::GeomGLS, mapping = mapping, data = data, stat = stat,
    position = position, show.legend = show.legend, inherit.aes = inherit.aes,
    params = list(na.rm = na.rm, warn = warn, ...)
  )
}

## This stat was created for use in plot_aggregate_biomass for the planktivores
# since the nmle package could not fit the ar1 models
